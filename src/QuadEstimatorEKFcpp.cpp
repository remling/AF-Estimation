...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  /* from https://github.com/clarisli/FCND-Estimation-CPP/blob/master/src/QuadEstimatorEKF.cpp since it prob cannot get any more elegant */

  
    Quaternion<float> pred = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
    pred.IntegrateBodyRate(gyro, dtIMU);
    
    float predictedPitch = pred.Pitch();
    float predictedRoll = pred.Roll();
    ekfState(6) = pred.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // all three sources had the same, this seems straight forward
    
    predictedState(0) = curState(0) + dt * curState(3);  // new x is x + x_dot * dt
    predictedState(1) = curState(1) + dt * curState(4);  // new y is y + y_dot * dt
    predictedState(2) = curState(2) + dt * curState(5);  // new z is z + z_dot * dt
    
    V3F accelI = attitude.Rotate_BtoI(accel);    // acceleration
    
    // new x_dot is x_dot + x_dot_dot * dt
    predictedState(3) = curState(3) + dt * accelI.x;
    
    // new y_dot is y_dot + y_dot_dot * dt
    predictedState(4) = curState(4) + dt * accelI.y;
    
    // new z_dot is z_dot + z_dot_dot * dt and account for z-ACC exluding gravity
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // from equ 52 of "Estimation for Quadrotors"

    float cT = cos(pitch); // cos(Theta)
    float sT = sin(pitch); // sin(Theta)
    
    float cH = cos(roll); // cos(Phi)  H since that is the first different letter to Psi
    float sH = sin(roll); // sin(Phi)
    
    float cS = cos(yaw);  // cos(Psi)  S since that is the first different letter to Phi
    float sS = sin(yaw);  // sin(Psi)

    RbgPrime(0,0) = -cT * sS;
    RbgPrime(0,1) = -sH * sT * sS - cT * cS;  // not sure if second term shoudl be cH*cS or cT*cS
    RbgPrime(0,2) = -cH * sT * sS + sH * cS;
    
    RbgPrime(1,0) = cT * cS;
    RbgPrime(1,1) = sH * sT * cS - cH * sS ;
    RbgPrime(1,2) = cH * sT * cS + sH * sS;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // from equ 51 of "Estimation for Quadrotors"
    ...
    // new Covariance from Algorithm 2 E(KF) in section 3 of "Estimation for Quadrotors"
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  // updating z and hprime  based on equs 53 and 55 of "Estimation for Quadrotors"
  for ( int i = 0; i < 6; i++) {
        zFromX(i) = ekfState(i);
        hPrime(i,i) = 1;
    }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  //  - The magnetommeter measurement covariance is available in member variable R_Mag
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
